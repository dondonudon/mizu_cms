...
const baseConfig = {
    ...
    userSuperAdminProp: 'isSuperAdmin',
    localStorageTokenName: 'RAFirebaseClientToken',
    handleAuthStateChange: async (auth, config) => {
        if (auth) {
            const snapshot = await firebase
                .firestore()
                .doc(config.userProfilePath + auth.user.email)
                .get();
            const profile = snapshot.data();

            if (profile && (profile[config.userAdminProp])) {
                const firebaseToken = await auth.user.getIdToken();
                const level = config.userAdminProp;
                let user = { auth, profile, firebaseToken, level };
                localStorage.setItem(config.localStorageTokenName, firebaseToken);
                console.log(config.userAdminProp);
                return user;
            } else if (profile && (profile[config.userSuperAdminProp])) {
                const firebaseToken = await auth.user.getIdToken();
                const level = config.userSuperAdminProp;
                let user = { auth, profile, firebaseToken, level };
                localStorage.setItem(config.localStorageTokenName, firebaseToken);
                console.log(config.userSuperAdminProp);
                return user;
            } else {
